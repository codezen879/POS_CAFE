...
async function main() {
  ...
  // --- Tax rates (GST: single combined rates used for menu tax) ---
  ...
  const catCoffee = await prisma.menuCategory.upsert({
    where: { slug: categories.coffee },
    update: {},
    create: { slug: categories.coffee, name: "Coffee", sortOrder: 1, icon: "☕" },
  });
  const catFood = await prisma.menuCategory.upsert({
    where: { slug: categories.food },
    update: {},
    create: { slug: categories.food, name: "Food & Snacks", sortOrder: 2, icon: "🥪" },
  });
  const catBakery = await prisma.menuCategory.upsert({
    where: { slug: categories.bakery },
    update: {},
    create: { slug: categories.bakery, name: "Bakery", sortOrder: 3, icon: "🥐" },
  });
  const catDesserts = await prisma.menuCategory.upsert({
    where: { slug: categories.desserts },
    update: {},
    create: { slug: categories.desserts, name: "Desserts", sortOrder: 4, icon: "🍰" },
  });
  const catBev = await prisma.menuCategory.upsert({
    where: { slug: categories.beverages },
    update: {},
    create: { slug: categories.beverages, name: "Beverages", sortOrder: 5, icon: "🧋" },
  });

  // --- Products (from spreadsheet: 101 Choco chip, 102 Extra foam, 103 Choco syrup + menu) ---
  const products = [
    ...
    { code: "402", name: "Vanilla Ice Cream", cat: catDesserts, price: 100, tax: tax12, veg: true, best: false, avail: true, prep: 0, max: null, desc: "Classic vanilla ice cream", requiresKitchen: false },
    ...
    { code: "503", name: "Coke", cat: catBev, price: 80, tax: tax12, veg: true, best: false, avail: true, prep: 0, max: null, desc: "Chilled packaged soft drink", requiresKitchen: false },
  ];
  ...
  for (const p of products) {
    await prisma.product.upsert({
      ...
      update: {
        ...
        requiresKitchen: p.requiresKitchen ?? true,
  ...
  // Attach syrup addons to coffee drinks
