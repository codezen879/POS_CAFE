...
| `OrderItem` | order_items | Snapshot of `name` + `unitPrice` + `quantity` + `note` (price/name snapshotted so later menu edits don't rewrite history). **Item-level pipeline fields**: `status` (`IN_PROCESS` default for new lines, then `READY`/`SERVED`/`DEFECTIVE`), `priority` (waiter fire-mark, blue), `readyAt` / `servedAt` / `defectiveAt` / `defectiveReason`. Indexed `[status, priority]` for the kitchen/waiter feeds. |
...
| PATCH/DELETE | `/api/products/:id` | manager | Update product (incl. `addonIds` replace; cross-category add-ons rejected 400) / delete |
...
| GET | `/api/pos/items` | staff | **Item-level kitchen/waiter feed**: flattened dish lines of all open-session, non-cancelled/draft orders (status, priority, table, order number, add-ons) |
| PATCH | `/api/pos/items/:itemId` | staff | Item transition (`IN_PROCESS`↔`READY`, →`SERVED`, →`DEFECTIVE`) + `priority` toggle; approved statuses only; guarded `updateMany` (race-safe, 409 on concurrent double-apply); idempotent replays; `DEFECTIVE` writes off waste (`ITEM_DEFECT`) and rolls the order status up |
