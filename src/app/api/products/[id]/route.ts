...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const {
    ...
    isVeg, isBestseller, isAvailable, requiresKitchen, prepTimeMins, maxOrderQty,
  } = body as any;

  if (requiresKitchen !== undefined && typeof requiresKitchen !== "boolean") {
    return Response.json({ error: "requiresKitchen must be a boolean" }, { status: 400 });
  }
  ...
  if (requiresKitchen !== undefined) data.requiresKitchen = requiresKitchen;
