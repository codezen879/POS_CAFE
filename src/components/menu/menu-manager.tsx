          ...
          return (
            <div key={p.id} className="rounded-xl border p-4">
              ...
              <div className="mt-2 flex flex-wrap gap-1">
                ...
                {p.requiresKitchen === false && <Badge variant="outline">Waiter direct</Badge>}
...
function ProductDialog({ mode, product, categories, taxRates, allAddons, onClose, onSaved }: any) {
  ...
  const [requiresKitchen, setRequiresKitchen] = useState(product?.requiresKitchen ?? true);
  ...
  async function save() {
    ...
    const payload = {
      ...
      taxRateId: taxRateId || null, isVeg, isBestseller, isAvailable, requiresKitchen,
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-lg">
        ...
        <div className="grid gap-3">
          ...
          <div className="flex items-center justify-between gap-4 rounded-lg border p-3">
            <div className="space-y-1">
              <Label htmlFor="requires-kitchen">Send to kitchen</Label>
              <p id="requires-kitchen-help" className="text-xs text-muted-foreground">
                When disabled, this item is served directly by the waiter.
              </p>
            </div>
            <Switch
              id="requires-kitchen"
              checked={requiresKitchen}
              onCheckedChange={setRequiresKitchen}
              aria-describedby="requires-kitchen-help"
            />
          </div>
          <div className="space-y-1">
