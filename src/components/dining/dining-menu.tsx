...
export function DiningMenu({ store, categories, table }: any) {
  ...
  const [sent, setSent] = useState<{
    count: number;
    tableName: string;
    awaitingReuseDecision: boolean;
    reuseOfferCount: number;
  } | null>(null);
  ...
  async function sendOrder() {
    ...
    try {
      ...
      setSent({
        ...
        awaitingReuseDecision: data.awaitingReuseDecision === true,
        reuseOfferCount: Number(data.reuseOfferCount || 0),
      });
      clearCart();
      setCartOpen(false);
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-background">
      ...
      {sent && (
        <div className="fixed inset-x-0 top-3 z-50 flex justify-center px-4">
          <div className="flex max-w-lg items-start gap-2 rounded-xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-medium text-emerald-700 shadow">
            <Check className="mt-0.5 h-4 w-4 shrink-0" />
            <span>
              {sent.awaitingReuseDecision
                ? `${sent.count} item${sent.count > 1 ? "s" : ""} received${sent.tableName ? ` for Table ${sent.tableName}` : ""}. ${sent.reuseOfferCount} ready-food match${sent.reuseOfferCount !== 1 ? "es are" : " is"} waiting for your waiter to confirm.`
