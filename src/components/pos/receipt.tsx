...
export function buildReceiptHtml(bill: ReceiptBill, store: any): string {
  ...
  for (const l of lines) {
    rows.push(`<div class="mt1">
      ...
      ${(l.addons || []).map((a) => {
        const totalAddonQty = a.quantity * l.quantity;
        return `<div class="row addon"><span>${esc(a.name)} ×${esc(totalAddonQty)}</span><span>${esc(formatCurrency(Number(a.price) * totalAddonQty))}</span></div>`;
      }).join("")}
    </div>`);
  }
